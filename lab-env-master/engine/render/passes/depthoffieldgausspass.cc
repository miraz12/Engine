...
namespace Passes
{
	DofGaussPass::DofGaussPass()
    {
		...
		composit = std::make_shared<Resources::ShaderObject>("content/Shader/dofgausspassY.vs", "content/Shader/dofgausspassY.fs");
		...
		composit->bind();
		gaussX->mod1i("inFullRes", 0);
		gaussX->mod1i("inDownSampled", 1);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, Servers::RenderServer::GetInstance()->width*0.5f, Servers::RenderServer::GetInstance()->height*0.5f, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
	...
	void DofGaussPass::UpdateResolution()
    {
		...
		v[5] = vector2D(9.4436f * dx, 0.0f);
		v[6] = vector2D(11.4401f * dx, 0.0f);
		gaussX->modVector2fArray("sampleArrayX", 12, v);
		...
		v[5] = vector2D(0.0f, 9.4436f * dy);
		v[6] = vector2D(0.0f, 11.4401f * dy);
		gaussY->modVector2fArray("sampleArrayY", 12, v);

		glBindTexture(GL_TEXTURE_2D, downTex);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, svr->width*0.5f, svr->height*0.5f, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
